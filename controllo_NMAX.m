...
%% Funzione che controlla che i valori del numero massimo di iterazione immessi dall'utente siano corretti
    ...
    % NMAX 1
         if (NMAX < 2)
             ...
             warning('WARN:NMAXI minore di 2','Il numero di iterazioni inserito è troppo piccolo, il numero minimo di iterazioni è 2 ');
         end
    ...
    % Segnalo se NMAX è grande
         if (NMAX > 1000)
            warndlg('Il numero di iterazioni inserito è molto alto, l''esecuzione potrebbe essere più lenta','Attenzione');
                 warning('WARN:NMAXI minore di 1000','Il numero di iterazioni inserito è molto alto, l''esecuzione potrebbe essere più lenta','Attenzione');
            uiwait(gcf);
         end
    end
